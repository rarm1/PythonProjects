...
class Email:
    ...
    def write_email(self, trade_table):
        body = f"{self.Salutation}, <br><br>" \
               f"We would like to place the following trades please,<b> these are to settle T+4</b>.<br><br>" \
               f"{trade_table}<br><br>" \
               f"Please don't hesitate to get in touch if there is any further information we are able to provide.<br><br>" \
               f"Best Regards, <br><br>"
        return body

    def save_email(self, trade_table):
        msg = MIMEMultipart()
        msg['Subject'] = self.subject
        msg['To'] = "; ".join(self.recipient_list)
        body = self.write_email(trade_table)
