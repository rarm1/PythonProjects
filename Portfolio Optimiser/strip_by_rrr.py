...
def process_weekly_returns(fund_returns):
	...
	for idx, weekly in enumerate(return_calculations):
		# This calculates returns by referencing the cell prior,
		# hence why it ensures there is a cell suitably far behind.
...
def filter_the_best(fully_sorted_companies: [list], companies_per_sector) -> list:
	...
	:rtype list of lists
	"""
	# filtered_companies = [classification[:5] if idx % 2 == 1 else classification[:10]
	# for idx, classification in enumerate(fully_sorted_companies, start=1)]
