...
def reg_coeff_calc(x, y):
    # When reading from Excel the default datatype is not compatiable with the stats library. This converts it.
    x = x.astype('float64')
    y = y.astype('float64')
    # Reshape -1, 1 transforms a horizontal array into a vertical array. This is important as it is the independant variable.
    x = np.array(x).reshape(-1, 1)
    # Similarly, y enters this function as a pandas dataframe, it must be a numpy array to be used by the stats library.
...
def benchmark_iterator(input_):
    ...
    y = input_[1]
    # The headers are imported from the document. Therefore the headers should be altered, if the output is to be altered.
