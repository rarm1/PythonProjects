...
# This generator creates an array. Moving this function allows for more processing without the function getting overly complex.
...
def trade_array_generator(idx, trade):
    ...
    if "broker" in notes.lower():
        ...
        return True, ['#{0}'.format(str(f'{tg_account_number_lookup()}')), ticker_lookup(isin), READER_SHEET.cell(4, 2).value, FUND_MANAGER,
                      trade_id, REQ_DATE,
...
# This function sets fund managers for third parties, or instructed trades. e.g. Clarion may instruct trades, but Margetts
# We do the rest of the paperwork. As such, Elizabeth is going to be the fund manager. Does this need additional oversight?
