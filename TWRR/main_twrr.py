...
# This checks whether the transaction should be considered.
...
def remove_duplicates(transaction_list):
	...
	:return: The given list w/out duplicates.
	"""
		...
		if res[1]:
			for key, val in holding_period.items():
				if not key.lower() in to_ignore:
					# Because there are multiple dates contained within the prices sheet and
					# the binary search only finds
					# the index of one, somewhere in this list, this range is given to allow for the selected cell to be
					# 200 away. Whilst creating a function that checks for each date span,
					# and dynamically presents a range
					# would be elegant. Computationally it is adding another step in processing,
					# rather than this assumption
...
def main():
	...
	user_requested_dates = user_transactions_getter(user_start_date, unit_time_series)
