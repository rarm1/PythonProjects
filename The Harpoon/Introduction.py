class Intro:
	...
	def write_introduction(self):
        ...
        {self.client_name}
        's portfolio has been back tested using the latest portfolio positioning provided.
        To compare the client’s portfolio to the Margetts Risk Rated
        {self.margetts_portfolio_identifier}
         strategy, we have assessed their geographical and asset class allocations.
         The asset allocation for the Margetts portfolio referenced in the following
         report refers to the long-term strategic asset allocation.
        {self.new_paragraph}
        Margetts have compared the back tested performance of the client’s portfolio
        using holdings and weightings provided to us on
        {self.date_provided}
        . This analysis does not reflect the performance achieved by the client’s account
        and only provides an indication of the current client portfolio’s expected risk
        and return characteristics. All performance data has been sourced from Morningstar.
        All performance figures have been run to
