...
SCHEME_LIST_LOCATION = 'Resources/scheme_list_desig.xlsm'
LIBERUM_ACC_NO_FILE = 'Resources/Liberum_Account_Numbers.csv'
...
TG_ACCOUNT_PATH = 'Resources/Thomas Grant account numbers.xlsx'
TG_ACCOUNT_DOC = xl.load_workbook(TG_ACCOUNT_PATH, data_only=True)
TG_ACCOUNT_NUMBERS = TG_ACCOUNT_DOC['Sheet1']
# The scheme loopup is written here. This is for ease of updating. Again, if multiple people are using this program, then
...
ISIN_TO_TICKER_PATH = 'Resources/ISIN_to_Ticker.xlsx'
...
def trade_array_generator(idx, trade):
    ...
    #     return True, get_broker_trade_details(trade, sec_name, isin, trade_id, notes), get_regular_trade_details(sec_name, isin, trade_id, trade, deal_type, trade_value, notes)
    # else:
...
def value_of_trade(deal_type, trade):
    ...
    if "sellall" in deal_type.lower():
        ...
        # This is where we need to have the live price of the ETF's so we can calculate the value of the trade in units.
        pass
...
def main_writer(trades):
    ...
    if len(trades) == 0:
        ...
        # Use the variables defined above to generate a csv document using functions that're built into the pandas repository
